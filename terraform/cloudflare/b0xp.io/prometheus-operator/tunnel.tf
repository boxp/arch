...
data "cloudflare_zero_trust_tunnel_cloudflared_token" "prometheus_operator_tunnel" {
  account_id = var.account_id
  tunnel_id  = cloudflare_zero_trust_tunnel_cloudflared.prometheus_operator_tunnel.id
}
resource "aws_ssm_parameter" "prometheus_operator_tunnel_token" {
  ...
  type        = "SecureString"
  value       = sensitive(data.cloudflare_zero_trust_tunnel_cloudflared_token.prometheus_operator_tunnel.token)
}
