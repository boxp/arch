...
resource "cloudflare_tunnel" "hermes_agent_tunnel" {
...
resource "cloudflare_tunnel_config" "hermes_agent_tunnel" {
  tunnel_id  = cloudflare_tunnel.hermes_agent_tunnel.id
  account_id = var.account_id
  config {
    ingress_rule {
      hostname = cloudflare_record.hermes_agent.hostname
...
resource "aws_ssm_parameter" "hermes_agent_tunnel_token" {
  ...
  type        = "SecureString"
  value       = sensitive(cloudflare_tunnel.hermes_agent_tunnel.tunnel_token)
}
