...
resource "cloudflare_dns_record" "grafana_api" { # Renamed from argocd_api
  zone_id = var.zone_id
  name    = "grafana-api.b0xp.io" # Changed from argocd-api
  # Points to the CNAME of the tunnel defined in tunnel.tf
  content = "${cloudflare_zero_trust_tunnel_cloudflared.grafana_api_tunnel.id}.cfargotunnel.com" # Changed from argocd_api_tunnel
  type    = "CNAME"
  ttl     = 1
