...
ARC が Lolice クラスタ上にデプロイ・稼働確認済み（2026-07-14）。

1. `apply-ansible.yml` / `plan-ansible.yml` の `runs-on` を `arc-runners` へ切り替え。
   - `test-ansible.yml` は QEMU/Docker を使用する Molecule テスト用のため `ubuntu-latest` を維持。
2. `boxp/is01-linux` の CI ワークフローも `arc-runners` へ切り替え（rate limit 解消）。
3. 動作確認・速度比較は ARC runner 上での初回 CI 実行で確認する。
