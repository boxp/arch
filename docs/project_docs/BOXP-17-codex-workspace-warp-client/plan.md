...
- WARP Access application は account 内で既に存在するため、新規作成ではなく既存の `type = "warp"` application を data source + import block で state 管理に寄せる。
- Cloudflare Zero Trust team name の初期値は、既存 WARP application の domain `boxp.cloudflareaccess.com/warp` に合わせて `boxp` とする。
...
- 2026-06-05: Terraform plan で既存 WARP application の domain が `boxp.cloudflareaccess.com/warp` であることを確認したため、SSM に保存する organization も `boxp` に更新する。
