...
**失敗 run #22478747958 で作成済み (state に残存)**:

| リソース | Terraform ID | 状態 |
|---|---|---|
| `tailscale_acl.this` | `dca925a2-b72b-a210-25af-3ccbdb1f4046` | Created (失敗 run で作成完了済み) |

**成功 run #22479672276 で追加作成 (3 added)**:
...
> **注**: 失敗した run #22478747958 では3リソースが並行作成された。`tailscale_acl.this` は
> 先に作成完了したが、`tailscale_tailnet_key` と `tailscale_federated_identity` は
> ACL の `tagOwners` 適用前にタグを参照したため HTTP 400 で失敗した。
> ACL は state に残存していたため、成功 run では残り3リソースのみが作成された。
