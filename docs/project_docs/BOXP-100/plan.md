...
- 通常 workspace では Novel Board role を設定せず、sidecar だけが image entrypoint から `boxp` user で runner loop を起動する。entrypoint は role 分岐前に home と private root の ownership・mode を初期化する。
