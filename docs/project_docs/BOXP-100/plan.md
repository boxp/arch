...
12. 稼働後の Pi 停止を調査し、CLI と vision model は正常だが実 run の subprocess が無通信・無出力のまま heartbeat だけを更新していたことを確認する。Pi を Novel 専用 prompt と built-in tools に隔離し、stdin を閉じ、全 agent route に hard timeout と process-tree cleanup を追加する。
13. timeout 時は親の終了前に子孫を捕捉し、grace period 後も生存する親・子孫へ強制終了を送り、全 process の終了を確認する。TERM を無視する子孫が timeout 後に原稿等を変更できないことを black-box test で検証する。
14. timeout 瞬間の固定 process tree では TERM handler が後から fork する子を捕捉できないため、agent を専用 process group で起動する。group 全体へ TERM、grace 後に KILL を送り、遅延 fork 子を含めて active process が残らないことを `/proc` と black-box test で確認する。
15. process group への signal が失敗しても cleanup を中断せず、`/proc` で観測済みの PID へ直接 signal を送り、grace 後の反復 KILL と残留確認まで必ず進む。group signal だけを失敗させる black-box test で子孫が残らないことを確認する。
16. agent の直接 process が正常終了した場合も、同じ process group の自然終了を grace 期間待つ。background child が残る場合は timeout と同じ TERM/KILL cleanup を適用し、exit 0 後に残留子が原稿等を変更できないことを black-box test で確認する。
17. agent ごとに一意な execution token を環境へ付与し、`/proc/*/environ` から同じ token を継承した process を反復検出する。agent が `setsid` / double-fork で process group 外へ detach した場合も TERM/KILL と残留確認の対象にし、timeout 後のファイル変更を black-box test で防止する。
18. execution token は子孫自身が環境から削除できるため、最終的な実行境界には使わない。Linux child-subreaper の専用 supervisor 配下で agent を起動し、環境変数除去、`setsid`、double-fork 後に孤児化した子孫も supervisor へ再親化して、全子孫の TERM/KILL・reap 完了まで supervisor が終了しない方式へ置き換える。token を除去して detach する black-box test で timeout 後の変更を防止する。
19. timeout cleanup 中は supervisor と全子孫の reap が完了するまで agent run を返さず、card lock の heartbeat を維持する。強制終了待ちの子孫が残る例外ケースでも同じカードの次 run を開始せず、旧 run と新 run が同じ作業原稿を並行更新しないようにする。
20. review feedback に従い、専用 supervisor の C++ 実装を廃止する。読みやすい Bash 実装が `tini -s` の Linux child-subreaper 境界下で `/proc` を反復走査し、既存と同じ timeout、TERM/KILL、全子孫消滅待ち、終了コード保持を担う構成へ置き換える。
...
- Pi が個人 extension／skill／AGENTS を読み込まず、vision model と built-in edit を非対話実行できること。停止した agent が設定時間後に exit 124 で終了し、lock を解放して人間確認レーンへ戻ること。
- 管理ノートの未検証 `published-path` が既存ファイルを横取りせず、正規の SFW/NSFW destination に公開されること。
- agent 実行中と Done の長時間 publish 中の heartbeat、active/stale lock、異常終了、lane/status 不一致、再起動後の lane 優先。
