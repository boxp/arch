...
本文 section は `Requirements`、`Outline`、`Review Instructions`、`Change History`、`Run History` を固定見出しとして用意する。runner は既存指示と履歴を追記し、削除しない。Pi に画像を渡す場合は `Requirements`、`Outline`、`Review Instructions` のいずれかに、vault 内 PNG/JPEG/GIF/WebP を Obsidian embed（`![[Attachments/example.png]]`）または Markdown image（`![alt](Attachments/example.png)`）で記載する。
...
冪等性は card lock と private `published.edn` に保存する `novel-id`、原稿 SHA-256、path、timestamp、reservation status を信頼境界として保証する。管理ノートの `published-path` だけでは既存ファイルを公開済みと判定せず、private state が同一 card・Done 時点の SFW/NSFW directory・reservation status と一致した場合だけ再利用する。コピー前に destination と原稿 SHA-256 を予約し、同じディレクトリの private staging file へコピーしてハッシュを検証した後、最終 path へ atomic move する。さらに canonical destination path 単位の OS file lock と永続 reservation を private root で共有し、別 process・別 card が同一分に同一 title を公開しても、存在確認から atomic move・確定までを直列化する。先に destination を予約した card だけが中断コピーを復旧でき、別 card は完成版を削除・置換しない。予約状態の最終 path が存在する場合も予約ハッシュを検証し、旧 runner の中断コピーによる不完全ファイルなら同じ予約 path へ完全な原稿を再配置する。`published` 状態のハッシュ不一致や未予約・他 card 予約済みの同名ファイルは上書きしない。Done 再走査やコピー直後の runner 再起動でも同一 ID の完成版は増えない。管理ノートには vault 相対 wiki link を残す。
...
| `pi` | `pi --print` | `CODEX_NOVEL_BOARD_PI_MODEL` 未設定時は Pi default。lolice 配備は `llama.cpp/gemma4-26b-vision` を明示 |

全 route は同じ prompt contract を受け、最後に `NOVEL_BOARD_RESULT: review` または `NOVEL_BOARD_RESULT: blocked` を返す。groom は exit 0 なら結果 marker に関係なく Draft、人間判断を必要とする執筆 run は Review へ戻す。全 route は `tini -s` の Linux child-subreaper 境界と、Bash で実装した専用 `novel-agent-supervisor` 配下で起動し、`CODEX_NOVEL_BOARD_AGENT_TIMEOUT_SECONDS`（既定7200秒）の hard timeout を持つ。子孫が環境変数を除去し、`setsid` / double-fork で別 process group へ detach しても、孤児化時には tini へ再親化され、Bash supervisor が `/proc` から反復検出するため実行境界から離脱できない。timeout 時は supervisor が観測する全子孫へ通常終了を送り、`CODEX_NOVEL_BOARD_AGENT_SHUTDOWN_GRACE_SECONDS`（既定10秒）後も残る子孫へ強制終了を反復し、全 process が終了・reap されるまで戻らない。cleanup 中は agent run を返さず card lock の heartbeat を維持するため、強制終了待ちの子孫が残る場合も同じカードの次 run は開始しない。direct agent process が正常終了した場合も同じ grace 期間だけ子孫の自然終了を待ち、background child が残れば同じ TERM/KILL cleanup を適用してから run 結果を確定する。これにより timeout 時の TERM handler が grace 中に fork した子孫、正常終了後に残された background child、環境変数を除去して process group 外へ detach した子孫を終了対象とする。全子孫の終了後、timeout は exit 124 として人間確認レーンへ戻す。

Pi は `--session-dir <run-dir>/pi-session`、`--approve`、任意の `--model` を使い、同じ原稿を filesystem から再開する。自動実行を Novel 専用 prompt と built-in file tools に限定するため、`--offline --no-extensions --no-skills --no-prompt-templates --no-context-files` を指定し、stdin は `/dev/null` とする。これにより home 上の個人 extension、skill、AGENTS.md、project-local resource、startup network operation が Novel run に混入しない。`--mode text` は stdout の出力形式であり入力モダリティの制限ではない。runner は管理ノートから解決できた画像を `@<absolute-path>` として Pi の初期 message に添付する。canonical path が vault 外になる参照、存在しない参照、basename が複数一致する曖昧な参照は添付しない。会話 session の永続性には依存しない。
...
- 管理ノートの frontmatter と Run History は ID 単位の OS file lock で runner 間を直列化し、並行 sync / publish が互いの記録を上書きしない。人間 editor や agent 自身はこの lock を取得しないため、review 指示保存後に assignee を変更する運用制約は残る。
...
- agent 実行中に加え、Done の完成版 publish（原稿・staging・完成版の SHA-256 計算とコピーを含む）の間も heartbeat を更新する。fresh lock は owner に関係なく回収しない。
...
- `CODEX_NOVEL_BOARD_AGENT_TIMEOUT_SECONDS`（既定 `7200`。全 agent route の hard timeout）
- `CODEX_NOVEL_BOARD_AGENT_SHUTDOWN_GRACE_SECONDS`（既定 `10`。timeout 後に supervisor 配下の子孫の通常終了を待つ秒数。経過後も active process があれば強制終了する）
...
black-box test は一時 vault と fake `codex` / `claude` / `pi` を使い、タイトルのみカードの採番・テンプレート scaffold・再走査時の重複防止、主要遷移、review stop/restart、全 route、Pi の自動実行隔離・vision model・vault-local `@image`・vault 外画像の拒否、agent timeout と process cleanup（TERM handler から遅延 fork する子、正常終了後の background child、実行 token を除去して `setsid` / double-fork する子を含む）、SFW/NSFW、冪等 publish、未検証 `published-path` の拒否、上書き禁止、別 process・別 card の同一 destination 競合、active/stale lock、lane/status sync、agent failure を再現する。
