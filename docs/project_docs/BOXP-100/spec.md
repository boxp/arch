...
codex-workspace image の既定 entrypoint は `CODEX_WORKSPACE_ROLE=novel-board-runner` の場合、通常 workspace の初期化へ進まず `novel_board_runner.bb loop` を `exec` する。配備側の独立 sidecar はこの role を設定して image の起動契約を使う。role 未設定時は通常 workspace のままであり、同一コンテナ内に runner を追加起動しない。
