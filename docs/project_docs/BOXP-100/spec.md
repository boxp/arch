...
全 route は同じ prompt contract を受け、最後に `NOVEL_BOARD_RESULT: review` または `NOVEL_BOARD_RESULT: blocked` を返す。groom は exit 0 なら結果 marker に関係なく Draft、人間判断を必要とする執筆 run は Review へ戻す。全 route は `setsid` で専用 process group に隔離し、`CODEX_NOVEL_BOARD_AGENT_TIMEOUT_SECONDS`（既定7200秒）の hard timeout を持つ。超過時は group 全体へ通常終了を送り、`CODEX_NOVEL_BOARD_AGENT_SHUTDOWN_GRACE_SECONDS`（既定10秒）後も active process があれば group 全体を強制終了する。direct agent process が正常終了した場合も同じ grace 期間だけ process group が空になるのを待ち、background child が残れば同じ TERM/KILL cleanup を適用してから run 結果を確定する。process group への signal が失敗した場合も、`/proc` で観測済みの PID への直接 signal を fallback として使い、反復 KILL と残留確認まで cleanup を継続する。これにより timeout 時の TERM handler が grace 中に fork した子孫と、正常終了後に残された background child の双方を終了対象とし、active process が残らないことを確認する。timeout は exit 124 として人間確認レーンへ戻す。
...
black-box test は一時 vault と fake `codex` / `claude` / `pi` を使い、タイトルのみカードの採番・テンプレート scaffold・再走査時の重複防止、主要遷移、review stop/restart、全 route、Pi の自動実行隔離・vision model・vault-local `@image`・vault 外画像の拒否、agent timeout と process-group cleanup（TERM handler から遅延 fork する子を含む）、SFW/NSFW、冪等 publish、未検証 `published-path` の拒否、上書き禁止、別 process・別 card の同一 destination 競合、active/stale lock、lane/status sync、agent failure を再現する。
