...
全 route は同じ prompt contract を受け、最後に `NOVEL_BOARD_RESULT: review` または `NOVEL_BOARD_RESULT: blocked` を返す。groom は exit 0 なら結果 marker に関係なく Draft、人間判断を必要とする執筆 run は Review へ戻す。全 route は Linux の `PR_SET_CHILD_SUBREAPER` を設定した専用 `novel-agent-supervisor` 配下で起動し、`CODEX_NOVEL_BOARD_AGENT_TIMEOUT_SECONDS`（既定7200秒）の hard timeout を持つ。子孫が環境変数を除去し、`setsid` / double-fork で別 process group へ detach しても、孤児化時には supervisor へ再親化されるため実行境界から離脱できない。timeout 時は supervisor が観測する全子孫へ通常終了を送り、`CODEX_NOVEL_BOARD_AGENT_SHUTDOWN_GRACE_SECONDS`（既定10秒）後も残る子孫へ強制終了を反復し、全 process を reap するまで終了しない。direct agent process が正常終了した場合も同じ grace 期間だけ子孫の自然終了を待ち、background child が残れば同じ TERM/KILL cleanup を適用してから run 結果を確定する。これにより timeout 時の TERM handler が grace 中に fork した子孫、正常終了後に残された background child、環境変数を除去して process group 外へ detach した子孫を終了対象とする。timeout は exit 124 として人間確認レーンへ戻す。
