...
2. **進捗把握の手動作業**: progressファイル（`.openclaw/workspace/progress/*.md`）を個別に確認する必要がある
3. **tmuxセッション間の移動コスト**: 複数のtmuxセッションを手動で切り替える必要がある
4. **エージェント状態のリアルタイム把握**: Claude Code/Codexの実行状態（実行中/入力待ち/完了）を即座に把握できない

### 1.2 cmux調査結果

[manaflow-ai/cmux](https://github.com/manaflow-ai/cmux) は macOS ネイティブのAIコーディングエージェント向けターミナルアプリケーションとして、同様の課題を解決している:

- **長所**: Claude Code hook統合が完全実装済み、Socket API、垂直タブUI
- **制約**: macOS/Swift依存（SwiftUI + AppKit + Ghostty）、Linux非互換、Codex統合未実装
- **アプローチ**: Bashラッパーによるhook自動注入 → ターミナルアプリ自体がペイン管理

参照: T-20260302-010（cmux進捗ビュー分析）、T-20260303-001（cmux調査タスク）
...
openclaw環境で複数のAIコーディングエージェントを並行運用するオペレーター（主にBOXP個人）。
...
- **エンドポイント**:
  - `POST /hook/session-start` — セッション開始通知
  - `POST /hook/notification` — 通知（進捗、質問、エラー等）
  - `POST /hook/stop` — セッション終了通知
...
- ceekerが停止してもエージェント動作に影響しない

#### 3.2.2 State Store

全worktreeの状態を集約するインメモリストア。

- **データソース**: Progress File Watcher, gwq status, Hook Receiver, tmux Query
- **更新トリガー**: inotifyイベント、hookイベント、定期ポーリング（gwq/tmux）
...
```json
{
  "hooks": {
    "SessionStart": [
      {
        "type": "command",
        "command": "curl -s -X POST http://127.0.0.1:9820/hook/session-start -H 'Content-Type: application/json' -d '{\"session_id\": \"$SESSION_ID\", \"cwd\": \"$CWD\"}'"
      }
    ],
    "Notification": [
      {
        "type": "command",
        "command": "curl -s -X POST http://127.0.0.1:9820/hook/notification -H 'Content-Type: application/json' -d '{\"session_id\": \"$SESSION_ID\", \"title\": \"$TITLE\", \"message\": \"$MESSAGE\"}'"
      }
    ],
    "Stop": [
      {
        "type": "command",
        "command": "curl -s -X POST http://127.0.0.1:9820/hook/stop -H 'Content-Type: application/json' -d '{\"session_id\": \"$SESSION_ID\", \"reason\": \"$REASON\"}'"
      }
    ]
  }
}
...
> **注意**: Codexのhook API仕様は今後変更される可能性がある。MVP段階ではClaude Code hookのみを対象とし、Codex hookはPhase 2以降で対応する。

### 5.4 hookが利用できない場合のフォールバック

hookが設定されていない、またはceekerのHTTPサーバーが起動していない場合でも、以下のデータソースで基本機能を提供:

1. **progressファイル監視**: `.openclaw/workspace/progress/*.md` のinotify監視
2. **gwq status**: worktree一覧と基本情報の取得
3. **tmux query**: `tmux list-sessions` / `tmux list-panes` による状態取得
4. **`.completed/*.done` マーカー**: タスク完了の検知
...
マッピング方式:
1. **cwd照合**: hookで受信したcwdとworktreeパスを照合
2. **tmux環境変数**: 各tmuxペインの環境変数から `$PWD` を取得して照合
3. **gwq status**: gwqの状態情報からworktree → tmuxセッション対応を取得
...
ceeker:
  1. 選択worktreeのtmux-targetを解決
     - SessionMapping から tmux session/window/pane を取得
     - 未登録の場合: tmux list-sessions + list-panes でcwd照合
  2. tmuxコマンド実行
...
| HTTPサーバー | ring + jetty（embedded） | hook receiver用、軽量 |
...
│   ├── hook/
│   │   ├── server.clj        ; HTTP hook receiver (ring + jetty)
...
**ゴール**: tmuxの左ペインで動作するworktree進捗モニタリングTUI
...
**非スコープ（MVP）**: Hook receiver, native image, Codex対応

### 8.2 Phase 2: Hook統合 + Native Image（目標: 2-3日追加）

**スコープ**:
- [ ] Hook receiver HTTPサーバー（ring + embedded jetty）
...
**受け入れ条件**:
- Claude Code hookイベントがTUIにリアルタイム反映されること
- native imageでの起動時間 < 100ms
- hookサーバーダウン時にClaude Codeの動作に影響しないこと
...
| Claude Code hook仕様の変更 | 中 | hookペイロードのパースを疎結合に実装。不明フィールドは無視する方針。 |
...
3. **ネットワーク**: hook受信にlocalhost HTTPポートを1つ使用（デフォルト: 9820）
...
3. openclaw, gwq, tmuxの動作に一切干渉しない設計のため、ロールバック時のデータ損失リスクなし
...
3. **gwq statusの出力形式**: JSON出力オプションの有無確認。テキストパースの場合はフォーマット安定性の確認。
4. **tmuxセッション命名規則**: gwqが作成するtmuxセッション名の規則確認。
5. **ポート番号（9820）**: 他サービスとの競合確認。
6. **Codex hook API**: Codex側のhook仕様が安定するまでPhase 2以降に先送り。

---

## 12. 参考資料

- [manaflow-ai/cmux](https://github.com/manaflow-ai/cmux) — macOS向けAIターミナルアプリ（cmux調査で分析）
- T-20260302-010: cmux進捗ビュー分析（`docs/project_docs/T-20260302-010/plan.md`）
- T-20260303-001: cmux調査タスク
- [ark-discord-bot](https://github.com/boxp/ark-discord-bot) — Clojure + GraalVM nativeのリファレンス実装
- openclaw plan: `docs/project_docs/openclaw/plan.md` — OpenClawデプロイ計画
- cc-sddガイドライン: `docs/project_docs/T-20260220-001-cc-sdd-guideline/plan.md` — AI支援開発ワークフロー
