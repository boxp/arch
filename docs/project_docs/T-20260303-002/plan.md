...
**MVP（Phase 1、hookなし）での一次解決戦略**:
1. **tmuxペインのcwd照合**（MVP primary）: `tmux list-panes -a -F '#{session_name}:#{window_index}.#{pane_index} #{pane_current_path}'` でペインのcwdを取得し、worktreeパスと照合する。hookに依存しない。
2. **gwq status**: gwqの状態情報からworktree一覧を取得し、パスベースでtmuxペインと紐付ける。

**Phase 2以降（hook利用可能時）の追加マッピング**:
...
ceeker:
  1. 選択worktreeのtmux-targetを解決
     [MVP] tmux list-panes -a でcwdを照合し、ペインを特定
     [Phase 2+] SessionMapping から tmux session/window/pane を取得（hookデータ利用）
     [フォールバック] いずれも見つからない場合: エラー表示
  2. tmuxコマンド実行
