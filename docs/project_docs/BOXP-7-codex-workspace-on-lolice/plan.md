...
- Even Terminal の認証 token は `/lolice/codex-workspace/even-terminal-token` SSM SecureString で固定し、Terraform は parameter の存在のみを管理して実値は `ignore_changes` で保持する。
- Dockerfile の pinned package versions は Renovate custom manager で更新対象にする。
- `terraform/cloudflare/b0xp.io/k8s` で既存 k8s tunnel に WARP private route `192.168.10.98/32` を追加する。
- `codex-workspace.b0xp.io` は DNS-only A record として `192.168.10.98` に解決させる。
- `lolice` 側は ARK server と同じ fixed LoadBalancer IP パターンで `192.168.10.98` の Service を作成し、SSH `2222` と Even Terminal `3456` を公開する。
...
- [x] Even Terminal token を固定する SSM SecureString placeholder を追加する。
- [x] Cloudflare WARP private route を追加する。
- [x] `codex-workspace.b0xp.io` を fixed LoadBalancer IP へ向ける。
- [x] private hostname route 方式をやめ、L4到達性を優先した private route 方式へ戻す。
...
- `terraform -chdir=terraform/aws/codex-workspace fmt -check`
- `terraform -chdir=terraform/aws/codex-workspace validate`
