...
| **glibc** | `cross` の既定イメージは新しい Ubuntu ベースで **glibc 2.38/2.39** に対してリンクする。実機の Debian bookworm は **2.36** なので起動できない | `Cross.toml` で Ubuntu 16.04 ベースの古いイメージに固定 |
...
# 上の表の 3 つの変換を適用してから
cross build --release -p codex-cli --target armv7-unknown-linux-gnueabihf
```
